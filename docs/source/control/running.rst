...
SLC ships with, a convenient :ref:`command line interface (CLI) <runner>` that lets you easily
...
The standard way to run a SLC algorithm from the command line is
...
.. admonition:: You Should Know
    ...
    If you are using ZShell: ZShell interprets square brackets as special characters. SLC uses square brackets
...
.. admonition:: Detailed Quickstart Guide
    ...
    The ``dt`` flag ensures that the save directory names will have timestamps in them (otherwise they don't, unless you set ``FORCE_DATESTAMP=True`` in :mod:`stable_learning_control.user_config`).

    The ``data_dir`` flag allows you to set the save folder for results. The default value is set by ``DEFAULT_DATA_DIR`` in :mod:`stable_learning_control.user_config`, which will be a subfolder
    ``data`` in the ``stable_learning_control`` folder (unless you change it).
...
If you run ``python -m stable_learning_control.run [algo]`` without ``_pytorch`` or ``_tf2``, the runner will look in ``stable_learning_control/user_config.py`` for which version it should
...
.. admonition:: You Should Know
    ...
    Values pass through :meth:`~stable_learning_control.control.utils.safer_eval.safer_eval()` before being used, so you can describe some functions and objects directly from
...
.. option:: --env, --env_name
    ...
    :obj:`str`. The name of an environment in gymnasium. All SLC algorithms are implemented as functions that accept ``env_fn`` as an argument, where ``env_fn``
...
.. option:: --data_dir
    ...
    :obj:`path str`. Set the base save directory for this experiment or set of experiments. If none is given, the ``DEFAULT_DATA_DIR`` in ``stable_learning_control/user_config.py`` will be used.
...
The CLI also contains several (shortcut) flags that can be used to change the behavior of the :class:`stable_learning_control.utils.log_utils.logx.EpochLogger`.
...
The SLC CLI comes with a handy configuration file loader that can be used to load `YAML`_ configuration files. These configuration files provide a convenient way to store your experiments'
...
.. admonition:: You Don't Actually Need to Know This One
    ...
    Each individual algorithm is located in a file ``stable_learning_control/algos/BACKEND/ALGO_NAME/ALGO_NAME.py``, and these files can be run directly from the command line
    with a limited set of arguments (some of which differ from what's available to ``stable_learning_control/run.py``). The command line support in thet in the individual algorithm files
...
There are two methods for adding custom environments to the SLC package. The first and easiest way is to make use of `OpenAi gym`_ it's internal module import
mechanism:
