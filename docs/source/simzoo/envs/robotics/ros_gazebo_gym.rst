...
The SLC package also works with the task environments contained in the `ros_gazebo_gym`_
...
installation and usage steps for using the SLC algorithms with the Panda task environment.
...
.. warning::
    ...
    task environments. If you want to use the SLC algorithms with any of the `ros_gazebo_gym`_  environments, you are
    advised to use the SLC package inside a virtual env. This virtual env can be created using
