"""Responsible for creating the CLI for the bayesian_learning_control package. It can
...
from bayesian_learning_control.user_config import DEFAULT_BACKEND
...
BASE_ALGO_NAMES = ["sac", "lac", "lac2", "lac3", "lac4", "lac5", "lac6", "sac2"]
...
def _parse_and_execute_grid_search(cmd, args):  # noqa: C901
    ...
    # warning
    algo = safer_eval("bayesian_learning_control.control." + cmd, backend=backend)

    # Before all else, check to see if any of the flags is 'help'.
    valid_help = ["--help", "-h", "help"]
    if any([arg in valid_help for arg in args]):
        print("\n\nShowing docstring for bayesian_learning_control." + cmd + ":\n")
        print(algo.__doc__)
        sys.exit()
    ...
    # to Bayesian Learning Control implementations, and may be hard to maintain.
...
def run(input_args):
    ...
    if cmd not in valid_cmds:
        raise ValueError(
            friendly_err(
                ...
                + "utility which is implemented in the bayesian_learning_control "
                "package."
            )
        )
    ...
    if cmd in valid_help:
        ...
        help_msg = (
            dedent(
                """
            Experiment in Bayesian Learning Control from the command line with

            \tpython -m bayesian_learning_control.run CMD [ARGS...]
        ...
        special_info = (
            ...
            \tpython -m bayesian_learning_control.run sac --env HalfCheetah-v2 --clip_ratio 0.1

            If you need a quick refresher on valid kwargs, get the docstring
            with

            \tpython -m bayesian_learning_control.run [algo] --help
        ...
        print("bayesian_learning_control, version {}".format(__version__))
