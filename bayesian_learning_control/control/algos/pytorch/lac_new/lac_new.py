...
.. note::
    Code Conventions:
        ...
        - When in the comments an equation (eq.) is specified without a prefix this regards an equation in `Han et al. 2020 <http://arxiv.org/abs/2004.14288>`_
"""  # noqa
...
class LAC_NEW(nn.Module):
    ...
    def update(self, data):  # noqa: C901
        ...
        if self.use_lyapunov:
            ...
            l_pi_ = self.ac.L(o_, a3_)
            l_lya_constrain = (l_pi_ - l1 + self._alpha3 * r).mean()
            ...
            # Constraint lyapunov error
            # l_error_constrained = self.labda * l_lya_constrain + l_error
            l_error_constrained = l_error

            l_error_constrained.backward()
            ...
            # labda_loss = -(
            #     self.log_labda * l_lya_constrain.detach()
        ...
        if self.use_lyapunov:
            ...
            # TODO: Clean up
            l1_pi = self.ac.L(o, pi)

            # a_loss = (
            #     self.labda.detach() * l_delta + self.alpha.detach() * logp_pi.mean()
            # )  # See eq. 12
            a_loss = (self.alpha.detach() * logp_pi - l1_pi).mean()  # See eq. 12
        else:
