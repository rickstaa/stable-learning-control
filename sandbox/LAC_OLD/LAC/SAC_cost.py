...
class SAC_cost(object):
    ...
    def learn(self, LR_A, LR_C, LR_L, batch):
        ...
        labda, alpha, q1_error, q2_error, l_error, entropy, a_loss = self.sess.run(
    ...
    def _build_a(self, s, name="actor", reuse=None, custom_getter=None):
        ...
        with tf.variable_scope(name, reuse=reuse, custom_getter=custom_getter):
            ...
            net_0 = tf.layers.dense(
                s, 256, activation=tf.nn.relu, name="l1", trainable=trainable
            )  # 原始是30
            net_1 = tf.layers.dense(
                net_0, 256, activation=tf.nn.relu, name="l4", trainable=trainable
            )  # 原始是30
            mu = tf.layers.dense(
    ...
    def _build_c(self, s, a, name="Critic", reuse=None, custom_getter=None):
        ...
        with tf.variable_scope(name, reuse=reuse, custom_getter=custom_getter):
            ...
            net_1 = tf.layers.dense(
                net_0, 256, activation=tf.nn.relu, name="l2", trainable=trainable
            )  # 原始是30
